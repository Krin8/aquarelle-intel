...
export async function findRobustDomainPattern(domain: string, samplePersons: Array<{ name: string; firstName: string; lastName: string }> = []): Promise<{ pattern: string, domain: string }> {
  ...
  for (const samplePerson of samplesToTry) {
    ...
    console.log(`[PatternFallback] Trying to derive pattern using sample person: ${samplePerson.name}`);

    // --- HUNTER.IO FALLBACK ---
    const hunterKey = process.env.HUNTER_API_KEY;
    if (hunterKey) {
      try {
        const hunterRes = await fetch(`https://api.hunter.io/v2/email-finder?domain=${domain}&first_name=${encodeURIComponent(samplePerson.firstName)}&last_name=${encodeURIComponent(samplePerson.lastName)}&api_key=${hunterKey}`);
        if (hunterRes.ok) {
          ...
          if (foundEmail && isFound) {
            ...
            const actualDomain = foundEmail.split('@')[1] || domain;
    ...
    if (findymailKey) {
      try {
        const findyRes = await fetch('https://app.findymail.com/api/search/name', {
          ...
          body: JSON.stringify({ name: samplePerson.name, domain })
        });
        if (findyRes.ok) {
    ...
    if (dcKey) {
      try {
        const dcRes = await fetch('https://api.dropcontact.com/v1/enrich/all', {
          ...
          body: JSON.stringify({ data: [{ first_name: samplePerson.firstName, last_name: samplePerson.lastName, website: domain }] })
        });
        if (dcRes.ok) {
    ...
    if (pdlKey) {
      try {
        const pdlRes = await fetch('https://api.peopledatalabs.com/v5/person/enrich', {
          ...
          body: JSON.stringify({ name: samplePerson.name, company: domain })
        });
        if (pdlRes.ok) {
    ...
    if (prospeoKey) {
      try {
        const prospeoRes = await fetch('https://api.prospeo.io/email-finder', {
          ...
          body: JSON.stringify({ first_name: samplePerson.firstName, last_name: samplePerson.lastName, company: domain })
        });
        if (prospeoRes.ok) {
